...
class GAN:
    def __init__(self):        
        ...
        self.noise = tf.placeholder(tf.float32, [None, 28, 28, 1])
    ...
    def Discriminator(self, input, reuse):
        with tf.variable_scope('dis') as scope:
            ...
            sub_conv1 = tf.layers.conv2d(inputs=input, filters=32, kernel_size=[3, 3],
                                      kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                      strides=[1, 1], padding='valid', activation=tf.nn.relu, name='sub_conv1')
            reshape = tf.reshape(sub_conv1, [-1, 26*26*32], name='reshape')
            hidden_1 = tf.layers.dense(inputs=reshape, units=256, activation=tf.nn.relu, name='hidden_1')
            output = tf.layers.dense(inputs=hidden_1, units=1, activation=tf.nn.sigmoid, name='output')
    ...
    def Generator(self, input):
        with tf.variable_scope('gen'):
            ...
            output = tf.layers.conv2d(inputs=sub_conv1, filters=1, kernel_size=[3, 3],
                                      kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                      strides=[1, 1], padding='same', activation=tf.nn.relu)
        params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='gen')
        return output, params

def get_noise(batch_size, n_noise_h, n_noise_w, n_noise_channel):
    return np.random.normal(size=(batch_size, n_noise_h, n_noise_w, n_noise_channel))
...
total_epoch = 300
n_noise_h, n_noise_w, n_noise_channel = 28, 28, 1

loss_val_D, loss_val_G = 0, 0
for epoch in range(total_epoch):
    for i in range(total_batch):
        batch_xs, batch_ys = mnist.train.next_batch(batch_size)
        noise = get_noise(batch_size, n_noise_h, n_noise_w, n_noise_channel)
